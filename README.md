# Enhanced Condenser Editor (ECE)

**Enhanced Condenser Editor (ECE)** is a browser extension designed to upgrade the writing experience on Condenser-based platforms (such as Steemit). It introduces a side-by-side markdown preview mode that works for both top-level posts and comment replies.

## Features

- **Side-by-Side Preview**: Real-time markdown preview positioned next to your editor.
- **Deep Integration**: Works seamlessly with post creators and reply boxes.
- **Responsive Layout**: Automatically hides sidebars and expands the editor to utilize the full width of your screen.
- **Multi-Domain Support**: Works across various Steemit development and community nodes.

## Supported Sites

- `steemit.com`
- `steemitdev.com`
- `moecki.online`
- `steemnet.org`

## Installation

1. Download or clone this repository.
2. Open Chrome and navigate to `chrome://extensions`.
3. Enable **Developer mode** in the top right.
4. Click **Load unpacked** and select the extension directory.

## License

This project is licensed under the MIT License - see the LICENSE file for details.